# HumanSignals Helm chart configuration

![Version: 30.28.0](https://img.shields.io/badge/Version-30.28.0-informational?style=flat-square) ![AppVersion: 1.43.0](https://img.shields.io/badge/AppVersion-1.43.0-informational?style=flat-square)

## Configuration

The following table lists the configurable parameters of the HumanSignals chart and their default values.
...
| notificationEmail | string | `nil` | Notification email for notifications to be sent to from the HumanSignals stack |
| siteUrl | string | `nil` |  |
| image.repository | string | `"humansignals/humansignals"` | HumanSignals image repository to use. |
| image.sha | string | `nil` | HumanSignals image SHA to use (example: `sha256:20af35fca6756d689d6705911a49dd6f2f6631e001ad43377b605cfc7c133eb4`). |
| image.tag | string | `nil` | HumanSignals image tag to use (example: `release-1.43.0`). |
| image.default | string | `":release-1.43.0"` | HumanSignals default image. Do not overwrite, use `image.sha` or `image.tag` instead. |
| image.pullPolicy | string | `"IfNotPresent"` | HumanSignals image pull policy. |
| image.pullSecrets | list | `[]` |  |
| sentryDSN | string | `nil` | Sentry endpoint to send errors to. |
| humansignalsSecretKey.existingSecret | string | `nil` | Specify that the key should be pulled from an existing secret key. By default the chart will generate a secret and create a Kubernetes Secret containing it. |
| humansignalsSecretKey.existingSecretKey | string | `"humansignals-secret"` | Specify the key within the secret from which SECRET_KEY should be taken. |
| env | list | `[]` | Environment variables to inject into every HumanSignals deployment. |
| migrate.enabled | bool | `true` | Whether to install the HumanSignals migrate job or not. |
| events.enabled | bool | `true` | Whether to install the HumanSignals events stack or not. |
...
| recordings.enabled | bool | `false` | Whether to install the HumanSignals recordings stack or not. |
...
| decide.enabled | bool | `false` | Whether to install the HumanSignals decide stack or not. |
...
| web.enabled | bool | `true` | Whether to install the HumanSignals web stack or not. |
...
| web.env | object | `{}` | Additional env variables to inject into the web stack deployment. |
| web.internalMetrics.capture | bool | `false` | Deprecated: Whether to capture information on operation of humansignals into humansignals, exposed in /instance/status page |
...
| worker.enabled | bool | `true` | Whether to install the HumanSignals worker stack or not. |
...
| plugins.enabled | bool | `true` | Whether to install the HumanSignals plugin-server stack or not. This service handles data ingestion into ClickHouse, running apps and async jobs. To scale these components separately, instead use the individual component deployments, pluginsIngestion, pluginsAsync, pluginsJobs, and pluginsScheduler. |
...
| pluginsAsync.enabled | bool | `false` | Whether to install the HumanSignals plugin-server async stack or not. If disabled (default), plugins service handles both ingestion and running of async tasks. Allows for separate scaling of this service. |
...
| pluginsIngestion.enabled | bool | `false` | Whether to install the HumanSignals plugin-server ingestion capability as an individual workload. |
...
| pluginsIngestionOverflow.enabled | bool | `false` | Whether to install the HumanSignals plugin-server ingestion overflow capability as an individual workload. |
...
| recordingsIngestion.enabled | bool | `false` | Whether to install the HumanSignals session recordings ingestion capability as an individual workload. |
...
| recordingsBlobIngestion.enabled | bool | `false` | Whether to install the HumanSignals session recordings blob ingestion capability as an individual workload. |
...
| pluginsExports.enabled | bool | `false` | Whether to install the HumanSignals plugin-server exports capability as an individual workload. |
...
| pluginsJobs.enabled | bool | `false` | Whether to install the HumanSignals plugin-server jobs capability as an individual workload. |
...
| pluginsScheduler.enabled | bool | `false` | Whether to install the HumanSignals plugin-server scheduler capability as an individual workload. |
...
| toolbox.enabled | bool | `false` | Whether to install the HumanSignals toolbox deployment |
...
| temporalPyWorker.enabled | bool | `false` | Whether to install the HumanSignals Temporal Python worker stack or not. |
...
| service.name | string | `"humansignals"` | HumanSignals service name. |
| service.type | string | `"NodePort"` | HumanSignals service type. |
| service.externalPort | int | `8000` |  |
| service.internalPort | int | `8000` |  |
| service.annotations | object | `{}` | HumanSignals service annotations. |
...
| ingress.hostname | string | `nil` | URL to address your HumanSignals installation. You will need to set up DNS after installation |
| ingress.gcp.ip_name | string | `"humansignals"` | Specifies the name of the global IP address resource to be associated with the google clb |
...
| postgresql.nameOverride | string | `"humansignals-postgresql"` | Name override for PostgreSQL app. |
| postgresql.postgresqlDatabase | string | `"humansignals"` | PostgreSQL database name. |
...
| redis.nameOverride | string | `"humansignals-redis"` |  |
...
| kafka.nameOverride | string | `"humansignals-kafka"` |  |
...
| kafka.externalZookeeper.servers | list | `["humansignals-humansignals-zookeeper:2181"]` | List of external zookeeper servers to use. |
...
| zookeeper.nameOverride | string | `"humansignals-zookeeper"` |  |
...
| clickhouse.cluster | string | `"humansignals"` | Clickhouse cluster |
| clickhouse.database | string | `"humansignals"` | Clickhouse database |
...
| clickhouse.existingSecret | string | `""` | Clickhouse existing secret name that needs to be in the namespace where humansignals is deployed into. Will not use the above password value if set |
...
| clickhouse.image.tag | string | `"22.8.11.15"` | ClickHouse image tag. Note: HumanSignals does not support all versions of ClickHouse. Please override the default only if you know what you are doing. |
...
| clickhouse.defaultSettings.default_database | string | `"humansignals"` |  |
...
| clickhouse.client.image.tag | string | `"22.8.11.15"` | ClickHouse image tag. Note: HumanSignals does not support all versions of ClickHouse. Please override the default only if you know what you are doing. |
...
| clickhouse.backup.clickhouse_services | string | `"chi-humansignals-humansignals-0-0"` |  |
...
| externalClickhouse.database | string | `"humansignals"` | Database name for the external cluster |
...
| minio.defaultBuckets | string | `"humansignals"` | Comma, semi-colon or space separated list of buckets to create at initialization (only in standalone mode) |
...
| grafana.sidecar | object | `{"dashboards":{"enabled":true,"folderAnnotation":"grafana_folder","label":"grafana_dashboard","provider":{"foldersFromFilesStructure":true}}}` | Sidecar configuration to automagically pull the dashboards from the `grafana-dashboard` folder. See [official docs](https://github.com/grafana/helm-charts/blob/main/charts/grafana/README.md) for more info. |
| grafana.datasources | object | `{"datasources.yaml":{"apiVersion":1,"datasources":[{"access":"proxy","isDefault":true,"jsonData":{"timeInterval":"60s"},"name":"Prometheus","type":"prometheus","url":"http://humansignals-prometheus-server"},{"access":"proxy","isDefault":false,"name":"Loki","type":"loki","url":"http://humansignals-loki-read:3100"},{"access":"proxy","isDefault":false,"jsonData":{"implementation":"prometheus"},"name":"Alertmanager","type":"alertmanager","url":"http://humansignals-prometheus-alertmanager"}]}}` | Configure Grafana datasources. See [docs](http://docs.grafana.org/administration/provisioning/#datasources) for more info. |
...
| loki.fullnameOverride | string | `"humansignals-loki"` |  |
| loki.nameOverride | string | `"humansignals-loki"` |  |
...
| promtail.config.clients[0].url | string | `"http://humansignals-loki-write:3100/loki/api/v1/push"` |  |
...
| promtail.config.snippets.pipelineStages[2].match.selector | string | `"{app=\"humansignals\", container=~\"humansignals-web|humansignals-worker|humansignals-events\"}"` |  |
...
| prometheus-kafka-exporter.kafkaServer | list | `["humansignals-humansignals-kafka:9092"]` | Specify the target Kafka brokers to monitor. |
...
| prometheus-postgres-exporter.config | object | `{"datasource":{"host":"humansignals-humansignals-postgresql","passwordSecret":{"key":"postgresql-password","name":"humansignals-humansignals-postgresql"},"user":"postgres"}}` | Configuration options. |
...
| prometheus-redis-exporter.redisAddress | string | `"redis://humansignals-humansignals-redis-master:6379"` | Specify the target Redis instance to monitor. |
