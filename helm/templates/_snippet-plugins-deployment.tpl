...
kind: Deployment
metadata:
  name: {{ template "humansignals.fullname" .root }}-{{ .name }}
...
spec:
  selector:
    matchLabels:
        app: {{ template "humansignals.fullname" .root }}
  ...
  template:
    metadata:
      ...
      labels:
        ...
        {{- end }}
    spec:
      serviceAccountName: {{ template "humansignals.serviceAccountName" .root }}
      ...
      - name: {{ .root.Chart.Name }}-{{ .name }}
        image: {{ template "humansignals.image.fullPath" .root }}
        ...
        {{- include "snippet.humansignals-env" .root | indent 8 }}
