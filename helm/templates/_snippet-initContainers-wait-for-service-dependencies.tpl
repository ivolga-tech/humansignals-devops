...
- name: wait-for-service-dependencies
  ...
  command:
    ...
    - >
        ...
        until (
            NODES_COUNT=$(wget -qO- \
                "http://$CLICKHOUSE_USER:$CLICKHOUSE_PASSWORD@{{ include "humansignals.clickhouse.fullname" . }}.$(cat /var/run/secrets/kubernetes.io/serviceaccount/namespace).svc.cluster.local:8123" \
        ...
        until (nc -vz "{{- default (printf "%s-pgbouncer" (include "humansignals.fullname" .)) .Values.pgbouncer.fqdn -}}" {{ include "humansignals.pgbouncer.port" . }});
        do
            echo "waiting for PgBouncer"; sleep 1;
        done

        {{ if .Values.postgresql.enabled }}
        until (nc -vz "{{ include "humansignals.postgresql.host" . }}.$(cat /var/run/secrets/kubernetes.io/serviceaccount/namespace).svc.cluster.local" {{ include "humansignals.postgresql.port" . }});
        do
        ...
        until (nc -vz "{{ include "humansignals.redis.host" . }}.$(cat /var/run/secrets/kubernetes.io/serviceaccount/namespace).svc.cluster.local" {{ include "humansignals.redis.port" . }});
        do
        ...
        KAFKA_BROKERS="{{ include "humansignals.kafka.brokers" . }}"
